...
def determine(inadd, outadd):

    #take 4 pictures
    motion_camera.picture(inadd)    

    #준비
    predictor = dlib.shape_predictor('/home/camel/Desktop/rpiMiniProject/services/shape_predictor_68_face_landmarks.dat')
    ...
    for add in range(0, 4):
        ...
        #얼굴
        img = cv2.imread(inadd[add]) #이미지 파일 불러오기
        dets = detector(img, 1)
        ...
        for k, d in enumerate(dets): 
            ...
            color_l_out = (0, 0, 0) #랜드마크 바깥쪽(out) - 파랑
            color_l_in = (255, 255, 255)    #랜드마크 안쪽(in) - 초록 
                ## 표시할 선, 도형
            line_width = 3
            circle_r = 3
                ## 글씨 
...
determine(input_address, output_address)